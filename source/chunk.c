...
void chunk_init(struct chunk* c, struct world* world, w_coord_t x, w_coord_t y,
    ...
    // 1. Zero-out the struct immediately to prevent garbage pointers if the next steps fail
    c->blocks = NULL;
    c->x = x;
    c->y = y;
    c->z = z;
    ...
    // 2. Safely attempt allocation
    c->blocks = malloc(CHUNK_SIZE * CHUNK_SIZE * CHUNK_SIZE * 5 / 2);
    if (!c->blocks) return; // Safely abort without leaving a garbage state
...
static void chunk_destroy(struct chunk* c) {
    ...
    // 1. Safe free and pointer nullification to prevent Use-After-Free
    if (c->blocks) {
        free(c->blocks);
        c->blocks = NULL; 
    }
...
struct block_data chunk_get_block(struct chunk* c, c_coord_t x, c_coord_t y,
                                  c_coord_t z) {
    if (!c || !c->blocks || x >= CHUNK_SIZE || y >= CHUNK_SIZE || z >= CHUNK_SIZE) {
        return (struct block_data) {0, 0, 0, 0};
    }
...
struct block_data chunk_lookup_block(struct chunk* c, w_coord_t x, w_coord_t y,
    ...
    if (other) {
        c_coord_t safe_x = (c_coord_t)((x % CHUNK_SIZE + CHUNK_SIZE) % CHUNK_SIZE);
        c_coord_t safe_y = (c_coord_t)((y % CHUNK_SIZE + CHUNK_SIZE) % CHUNK_SIZE);
        c_coord_t safe_z = (c_coord_t)((z % CHUNK_SIZE + CHUNK_SIZE) % CHUNK_SIZE);

        return chunk_get_block(other, safe_x, safe_y, safe_z);
    } else {
        return (struct block_data) {
...
void chunk_set_light(struct chunk* c, c_coord_t x, c_coord_t y, c_coord_t z,
                     uint8_t light) {
    if (!c || !c->blocks || x >= CHUNK_SIZE || y >= CHUNK_SIZE || z >= CHUNK_SIZE) return;
