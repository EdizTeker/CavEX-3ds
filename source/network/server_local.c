...
// CRITICAL FIX: The Bad Chunk Blacklist
#define MAX_BAD_CHUNKS 256
static w_coord_t bad_x[MAX_BAD_CHUNKS];
static w_coord_t bad_z[MAX_BAD_CHUNKS];
...
static void server_local_update(struct server_local* s) {
    ...
    for(w_coord_t z = pz - MAX_VIEW_DISTANCE; z <= pz + MAX_VIEW_DISTANCE; z++) {
        for(w_coord_t x = px - MAX_VIEW_DISTANCE; x <= px + MAX_VIEW_DISTANCE; x++) {
            w_coord_t d = CHUNK_DIST2(px, x, pz, z);
            
            // Check if chunk is blacklisted
            bool is_bad = false;
            for (int i = 0; i < bad_count; i++) {
                if (bad_x[i] == x && bad_z[i] == z) {
                    is_bad = true;
                    break;
                }
            }

            if(!is_bad && !server_world_is_chunk_loaded(&s->world, x, z)
    ...
    if(c_nearest) {
        if (server_world_load_chunk(&s->world, c_nearest_x, c_nearest_z, &sc)) {
            ...
            if (!ids || !metadata || !lighting_sky || !lighting_torch) {
                ...
                if (lighting_sky) free(lighting_sky);
                if (lighting_torch) free(lighting_torch);
                return;
            }
            ...
            // THE TRIPWIRE: Add broken chunk to blacklist so we never touch it again.
            if (bad_count < MAX_BAD_CHUNKS) {
                bad_x[bad_count] = c_nearest_x;
                bad_z[bad_count] = c_nearest_z;
                bad_count++;
                printf("[SERVER] Blacklisted Corrupt Chunk at X:%d Z:%d\n", (int)c_nearest_x, (int)c_nearest_z);
                fflush(stdout);
            }
        }
